    ...
    *,
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            if e.code in _RETRYABLE_HTTP_CODES and attempt < attempts:
                delay = min(20.0, 2.0 ** (attempt - 1))
                logger.info("Gemini HTTP %s, %.1fs 후 재시도 (%s/%s)", e.code, delay, attempt, attempts)
                time.sleep(delay)
                continue
            msg = err_body[:800]
            try:
            ...
            if attempt < attempts:
                delay = min(20.0, 2.0 ** (attempt - 1))
                logger.info("Gemini 연결 실패, %.1fs 후 재시도 (%s/%s)", delay, attempt, attempts)
                time.sleep(delay)
                continue
            raise GeminiApiError(f"연결 실패: {e.reason}") from e
    else:
        if last_http_error is not None:
            raise GeminiApiError(f"HTTP {last_http_error.code}: {last_http_error.reason}") from last_http_error
        if last_url_error is not None:
            raise GeminiApiError(f"연결 실패: {last_url_error.reason}") from last_url_error
        raise GeminiApiError("Gemini 요청 실패")
